...
- **Архитектура (выбрана по докладу):** NestJS-сервисы — **Hexagonal (Ports & Adapters) + DDD
  tactical**, прагматично по сложности (`domain/application/infrastructure/interface`, модуль =
  composition root, биндит port-токены → адаптеры; use-cases тестируются с in-memory адаптерами).
  Микрофронты — **MVVM** (ViewModel-хуки `model/` + чистые View `ui/`) + **SCSS Modules**, без
  инлайн-стилей. Макро-уровень (микросервисы/EDA/outbox/MF/Keycloak) не меняем. Те же подходы
  применять к Python-сервисам (Срез 2).

## Стек (пинить свежие стабильные патчи; ориентир — июнь 2026)

- Монорепо: **Nx 22.x**; пакетный менеджер — **pnpm 11** (через corepack), НЕ npm.
- Frontend: **React 19.2** + **Rspack** + **Module Federation 2.0** (`@module-federation/enhanced`); **React Router 7**, **TanStack Query 5**, **SCSS Modules**.
...
- Node **24 LTS**; ORM **Prisma 7** (generator `prisma-client` → generated-клиент в `src/generated/prisma`; connection string в `prisma.config.ts`; рантайм через driver adapter `@prisma/adapter-pg`); SRS — **ts-fsrs**.
...
- [x] Срез 1 — walking skeleton (identity, vocabulary, learning, gateway-bff, shell, mfe-learner; Kafka+outbox; Keycloak; compose). Код готов: все юнит-тесты + e2e на основной флоу зелёные, оба микрофронта собираются (MF 2.0), сервисы typecheck. Живой прогон `docker compose up` + браузерный флоу — по README.
