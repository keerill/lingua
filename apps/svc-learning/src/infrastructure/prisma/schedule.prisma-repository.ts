...
/** Prisma adapter for {@link ScheduleRepository}. */
@Injectable()
export class PrismaScheduleRepository implements ScheduleRepository {
  ...
  async saveInitial(schedule: Schedule): Promise<void> {
    ...
    await this.prisma.cardSchedule.upsert({
      where: { userId_cardId: { userId: schedule.userId, cardId: schedule.cardId } },
      create: {
      ...
      update: {}, // already exists → keep current schedule (idempotent)
    });
  }

  async findByUserAndCard(userId: string, cardId: string): Promise<Schedule | null> {
    const row = await this.prisma.cardSchedule.findUnique({
      where: { userId_cardId: { userId, cardId } },
    });
    return row ? Schedule.fromSnapshot(userId, cardId, this.toSnapshot(row)) : null;
  }
  ...
  async findDue(userId: string, limit: number): Promise<Schedule[]> {
    ...
    return rows.map((r) => Schedule.fromSnapshot(userId, r.cardId, this.toSnapshot(r)));
  }
