...
/**
 * Primary adapter: consumes `vocabulary.card.created` and drives the
 * CreateInitialSchedule use case. Idempotent — safe under at-least-once delivery.
 */
@Injectable()
export class CardCreatedConsumer implements OnApplicationBootstrap, OnModuleDestroy {
  private readonly logger = new Logger(CardCreatedConsumer.name);
  private readonly consumer: KafkaConsumer;

  constructor(private readonly createInitialSchedule: CreateInitialScheduleUseCase) {
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    await this.consumer.subscribe([Topics.VocabularyCardCreated], async ({ value }) => {
