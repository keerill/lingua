...
/**
 * Internal API (called by the gateway-bff, not the browser). Identity arrives
 * in `x-owner-id`. Thin primary adapter — delegates to use cases.
 */
@Controller('internal')
export class VocabularyController {
  ...
  @Post('decks')
  createDeckHandler(@OwnerId() ownerId: string, @Body() dto: CreateDeckDto): Promise<Deck> {
  ...
  @Get('cards')
  getCardsHandler(@OwnerId() ownerId: string, @Query('ids') ids?: string): Promise<Card[]> {
