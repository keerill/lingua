...
import { DueScheduleRow, LearningPort } from '../../application/ports/learning.port';

/** HTTP adapter implementing {@link LearningPort} against svc-learning. */
@Injectable()
export class LearningHttpClient implements LearningPort {
  ...
  async getQueue(userId: string, limit: number): Promise<DueScheduleRow[]> {
    const { data } = await this.http.get<DueScheduleRow[]>('/internal/reviews/queue', {
      ...this.headers(userId),
      params: { limit },
    });
    return data;
  ...
  async submitReview(userId: string, cardId: string, grade: ReviewGrade): Promise<NextSchedule> {
