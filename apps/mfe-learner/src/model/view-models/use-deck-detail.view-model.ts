...
/**
 * ViewModel for the deck-detail (add cards) screen. Each created card triggers
 * `vocabulary.card.created` → FSRS schedule downstream.
 */
export function useDeckDetailViewModel(deckId: string) {
  ...
  const addCard = useMutation({
    mutationFn: () =>
      api.createCard(deckId, { term, translation, example: example || undefined }),
    onSuccess: (card) => {
