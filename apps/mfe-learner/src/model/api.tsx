...
/** Raw authenticated fetch injected by the shell host. */
export type ApiFetch = (path: string, init?: RequestInit) => Promise<Response>;

/** Typed BFF client built on top of the injected fetch (the MVVM "model" port). */
...
export function ApiProvider({ api, children }: { api: ApiFetch; children: ReactNode }) {
  const client = useMemo<LinguaApi>(
    () => ({
      ...
      createDeck: (dto) =>
        api('/decks', { method: 'POST', body: JSON.stringify(dto) }).then(json<Deck>),
      createCard: (deckId, dto) =>
        api(`/decks/${deckId}/cards`, { method: 'POST', body: JSON.stringify(dto) }).then(
          json<Card>,
        ),
      getQueue: (limit = 20) => api(`/reviews/queue?limit=${limit}`).then(json<DueCard[]>),
      submitReview: (cardId, grade) =>
        api(`/reviews/${cardId}`, { method: 'POST', body: JSON.stringify({ grade }) }).then(
          json<NextSchedule>,
        ),
    }),
    [api],
